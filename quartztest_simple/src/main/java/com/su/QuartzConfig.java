...
/**
 * 参考 :https://blog.csdn.net/KokJuis/article/details/78526709
 */
@Configuration
public class QuartzConfig
{
  ...
  // 定时任务组名称
  public static final String Quartz_Group_Name = "enjoyQuartzJobGroup1";
  //定时任务方法后缀
      ...
      throws ParseException
  {
    ...
    l.add(createTrigger(JobRefundWeichartBean.class, "0/10 * * * * ?"));
    l.add(createTrigger(OrderTimeoutJob.class, "0/10 * * * * ?"));
  ...
  private CronTriggerImpl createTrigger(Class<? extends Job> t, String cronExpression)
      throws ParseException
  {
...
//      throws IOException, ParseException, SchedulerException
  {
    ...
    // 应用启动完10秒后 QuartzScheduler 再启动
    factory.setStartupDelay(10);
    // 直接使用配置文件,用于quartz集群,加载quartz数据源配置,可以设置,也可以去掉,自动配置
    ...
    // 直接使用配置文件
    factory.setTriggers(createTriggers());
  ...
  /**
   * 添加该方法的目的在于一个使用场景。如果代码中删除了不需要的定时任务，但是数据库中不会删除掉，会导致之前 的定时任务一直在运行，如果把定时任务依赖的类删除了，就会导致报错，找不到目标。所以配置动态删除任务
   */
  @Bean
  public String fulsh(
      ...
      throws SchedulerException
  {
    ...
    try
    {
      ...
      // 现有数据库中已有的任务
      Set<TriggerKey> myGroupTriggers = s
          .getTriggerKeys(GroupMatcher.triggerGroupEquals(Quartz_Group_Name));
